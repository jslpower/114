		...
		public byte[] Encrypt(byte[] plainText, byte[] optionalEntropy) 
		{
			...
			try 
			{
				try 
				{
					...
					{
						throw new System.Exception("Unable to allocate plaintext buffer.");
					}
				...
				{
					throw new System.Exception("Exception marshalling data. " + ex.Message);
				}
				if(Store.Machine == store) 
				{
					...
					try 
					{
						...
						{
							throw new System.Exception("Unable to allocate entropy data buffer.");
						}
					...
					{
						throw new System.Exception("Exception entropy marshalling data. " + ex.Message);
					}
				}
				else 
				{
				...
				if(false == retVal) 
				{
					throw new System.Exception("Encryption failed. " + GetErrorMessage(Marshal.GetLastWin32Error()));
				}
			}
			catch(System.Exception ex) 
			{
				throw new System.Exception("Exception encrypting. " + ex.Message);
			}
		...
		public byte[] Decrypt(byte[] cipherText, byte[] optionalEntropy) 
		{
			...
			try 
			{
				try 
				{
					...
					{
						throw new System.Exception("Unable to allocate cipherText buffer.");
					}
				...
				if(Store.Machine == store) 
				{
					...
					try 
					{
						...
						if(IntPtr.Zero == entropyBlob.pbData) 
						{
							throw new System.Exception("Unable to allocate entropy buffer.");
						}
						entropyBlob.cbData = bytesSize;
						Marshal.Copy(optionalEntropy, 0, entropyBlob.pbData, bytesSize);
					}
				...
				if(false == retVal) 
				{
					throw new System.Exception("Decryption failed. " + GetErrorMessage(Marshal.GetLastWin32Error()));
				}
			...
			{
				throw new System.Exception("Exception decrypting. " + ex.Message);
			}
