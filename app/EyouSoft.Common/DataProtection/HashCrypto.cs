...
namespace EyouSoft.Common.DataProtection
{
	...
	public class HashCrypto:IDisposable,IHashCrypto
	{
		...
		public string SHAEncrypt(string inputString,int HashLength)
		{
			...
			{
				throw new System.Exception("SHA加密过程中发生错误:" + e.Message);
			}
		}
		/// <summary>
		/// DES 加密
			...
			try
			{
				...
				else
				{
					if(_Key.Length != 16 && _Key.Length != 24)
						throw new System.Exception("加密数据出错,详细原因：Key的长度不为 16 或 24 byte.");
				}
				if(null == this._IV)
				{
					des3.GenerateIV();
				...
				{
					if(_IV.Length != 8)
						throw new System.Exception("加密数据出错,详细原因：IV的长度不为 8 byte.");
				}
				//return _Key + "：" + Encoding.ASCII.GetBytes(_Key).Length.ToString() + "<br>"+ _IV + "：" + Encoding.ASCII.GetBytes(this._IV).Length.ToString();
				byt = Encoding.UTF8.GetBytes(Values);
		...
		public string RijndaelEncrypt(string Values)
		{
			try
			{
				...
				{
					if(_Key.Length != 32)
						throw new System.Exception("加密数据出错,详细原因：Key的长度不为 32 byte.");
				}
				if(null == this._IV)
				{
					rijndael.GenerateIV();
				...
				{
					if(_IV.Length != 16)
						throw new System.Exception("加密数据出错,详细原因：IV的长度不为 16 byte.");
				}
				//return _Key + "：" + Encoding.ASCII.GetBytes(_Key).Length.ToString() + "<br>"+ _IV + "：" + Encoding.ASCII.GetBytes(this._IV).Length.ToString();
				byt = Encoding.UTF8.GetBytes(Values);
		...
		public string RC2Encrypt(string Values)
		{
			try
			{
				...
				if(null == this._Key)
				{
					...
					if(_Key.Length != 16)
						throw new System.Exception("加密数据出错,详细原因：Key的长度不为 16 byte.");
				}
				if(null == this._IV)
				{
					rc2CSP.GenerateIV();
