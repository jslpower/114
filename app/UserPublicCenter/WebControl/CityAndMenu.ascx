<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="CityAndMenu.ascx.cs"
    Inherits="UserPublicCenter.WebControl.CityAndMenu" %>
...
<script type="text/javascript">
    $(function() {
        ...
        $(".sousuo_1 a:eq(0)").click(function() {
            ...
            var url = "/TourManage/TourList.aspx?SearchType=More&keyWord=" + escape(keyWord);
        ...
        $(".sousuo_1 a:eq(1)").click(function() {
            ...
            var keyWord = $.trim($("#<%=txtKeyWord.ClientID %>").val()) == "请输入关键词" ? "" : $.trim($("#<%=txtKeyWord.ClientID %>").val());
            var url = "/FindRival/rival.aspx?cId=" + CityId + "&provinceID=" + PrivoinceId + "&companyname=" + escape(keyWord);
            window.location.href = url;
            return false;
        });
