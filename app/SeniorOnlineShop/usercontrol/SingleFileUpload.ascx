<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="SingleFileUpload.ascx.cs" Inherits="SeniorOnlineShop.usercontrol.SingleFileUpload" %>
		...
		//$(function () {
		   //alert("<%=this.ClientID %>");
			    <%=this.ClientID %> = new SWFUpload({
				    // Backend Settings
				    upload_url: "<%=EyouSoft.Common.Domain.FileSystem%>/SeniorOnlineShop/upload.aspx?rd="+Math.random(),
				    file_post_name:"Filedata",
                    post_params : {
                        "ASPSESSID" : "<%=Session.SessionID %>",
                        "<%=EyouSoft.Security.Membership.UserProvider.LoginCookie_SSO %>": "TeamCustomization_SSO",
                        "<%=EyouSoft.Security.Membership.UserProvider.LoginCookie_UserName %>":"TeamCustomization_U",
                        "ImageWidth":"0",
                        "ImageHeight":"0",
                        "filetype":"worddoc",
				    ...
				    file_types : "*.jpg;*.png;*.gif;*.jpeg;*.doc;*.docx;*.xls;*.xlsx<%=!_IsUploadSwf?"":";*.swf" %>",
				    file_types_description : "文件<%=!_IsUploadSwf?"":"或者Falsh文件" %>",
