<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ServiceStandardList.aspx.cs"
    Inherits="SiteOperationsCenter.LineManage.ServiceStandardList" %>
    ...
    <script language="javascript" type="text/javascript">
        ...
        $("#btnAdd").click(function(){
            ...
            if(txtAddValue==""){
             ...
             var CompanyidAndUserid='<%=CompanyidAndUserid %>';
             var companyid= CompanyidAndUserid.toString().split('$')[0];
             var userid=CompanyidAndUserid.toString().split('$')[1];
             if(companyid=="0"||userid=="0")
             {
                alert("没有专线商或者联系人,请先选择专线商或者联系人");
                return false;
                }

                $.ajax({
	            type:"POST",
	            url:"/LineManage/ServiceStandardList.aspx?flag=add&CompanyidAndUserid="+CompanyidAndUserid+"&AddValue="+ encodeURI(txtAddValue) +"&Type=<%=ServiceType %>&rnd="+ Math.random(),
	            cache:false,
        ...
        $("#btnSave").click(function(){
            ...
            if(arr.length != 0)
            {
                ...
                if(arrContent != null)
                {
                    ...
                    if(parentVal != '')
                    {
                        parent.$("#<%=ContainerID %>").find("textarea[name="+ parentControlID +"]").val(parentVal + ";    " + arrContent.join(";    "));
                    }else{
                        parent.$("#<%=ContainerID %>").find("textarea[name="+ parentControlID +"]").val(arrContent.join(";    "));
                    }
                    var frameid=window.parent.Boxy.queryString("iframeId")
                    window.parent.Boxy.getIframeDialog(frameid).hide();
                }
