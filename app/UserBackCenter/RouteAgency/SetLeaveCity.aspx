<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="SetLeaveCity.aspx.cs" Inherits="UserBackCenter.RouteAgency.SetLeaveCity" %>
    ...
    <script type="text/javascript">
        $("#btnSave").click(function() {
            ...
            if (arr.length != 0) {
                ...
                $.ajax({
                    type: "POST",
                    url: "/RouteAgency/SetLeaveCity.aspx?inputclass=<%=inputclass %>&GetType=<%=GetType %>&flag=save&IdList=" + encodeURI(arr.join(',')) + "&NameList=" + encodeURI(arrName.join(',')) + "&ReleaseType=<%=ReleaseType %>&ContainerID=<%=ContainerID %>&rnd=" + Math.random(),
                    cache: false,
                    success: function(html) {
                        if (html != "") {
                            alert("设置成功!");
                            parent.$("#<%=Key %> #<%=ContainerID %>").closest("td").find("span").eq(0).html(html);
