...
*******************************************************************************/
/*2011/3/10 xuty 修改图片上传跨域处理,添加带上传图片的菜单样式项*/

var keSimple = ['source', '|', 'cut', 'copy', 'paste',
		      'wordpaste', '|', 'justifyleft', 'justifycenter', 'justifyright',
		     'title', 'fontname', 'fontsize', 'textcolor', 'bgcolor', 'bold', 'italic', 'underline', 'advtable',
			 'removeformat', '|', 'link', 'unlink'];
var keSimple_Img = ['source', '|', 'cut', 'copy', 'paste',
		      'wordpaste', '|', 'justifyleft', 'justifycenter', 'justifyright',
		     'title', 'fontname', 'fontsize', 'textcolor', 'bgcolor', 'bold', 'italic', 'underline', 'advtable',
			 'removeformat', '|', 'link', 'unlink','image'];			 
var keMore = ['source', '|', 'undo', 'redo', '|', 'cut', 'copy', 'paste', 'plainpaste', 'wordpaste', '|',
             'justifyleft', 'justifycenter', 'justifyright', 'justifyfull', 'insertorderedlist', 'insertunorderedlist', 'indent', 'outdent', 'subscript',
		     'superscript', '-', 'title', 'fontname', 'fontsize', 'textcolor', 'bgcolor', 'bold', 'italic', 'underline', 'advtable',
			 'removeformat', '|', 'link', 'unlink'];
var keMoreImg = ['source', '|', 'undo', 'redo', '|', 'cut', 'copy', 'paste', 'plainpaste', 'wordpaste', '|',
             'justifyleft', 'justifycenter', 'justifyright', 'justifyfull', 'insertorderedlist', 'insertunorderedlist', 'indent', 'outdent', 'subscript',
		     'superscript', '-', 'title', 'fontname', 'fontsize', 'textcolor', 'bgcolor', 'bold', 'italic', 'underline', 'advtable',
			 'removeformat', '|', 'link', 'unlink', 'image'];
    ...
    KE.plugin['image'] = {
        ...
        exec: function(id) {
            ...
            if (type == 2) {
                ...
                var onloadFunc = function(backMess) {
                    KE.event.remove(uploadIframe, 'load', mainLoadFun);
                    KE.util.hideLoadingPage(id);
                    // var uploadDoc = KE.util.getIframeDoc(uploadIframe);//---------------------------------
                    var data = '';
                    try {
                        //data = KE.util.parseJson(backMess);
                        data = eval("(" + backMess + ")");
                    }
                    catch (e) {
                ...
                var state = 1; //iframe加载步骤状态
                function mainLoadFun() {
                    state++;
                    if (state == 2) {//如果返回了url则改变iframe的location为当前同域名下的空白页
                        var blankUrl = "blankpage.html";
                        if (typeof KE.g[id].blankPageUrl != 'undefined')
                            blankUrl = KE.g[id].blankPageUrl;
                        uploadIframe.contentWindow.location = blankUrl;
                        return;
                    }
                    if (state == 3) {//如果返回了url且重新加载了同域名下的空白页则获取url执行插入
                        onloadFunc(uploadIframe.contentWindow.name);
                        state = 1;
                    }
                }
                KE.event.add(uploadIframe, 'load', mainLoadFun);
