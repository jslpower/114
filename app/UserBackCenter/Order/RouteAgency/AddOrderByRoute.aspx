<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="AddOrderByRoute.aspx.cs"
    Inherits="UserBackCenter.Order.RouteAgency.AddOrderByRoute" %>

<%@ Import Namespace="EyouSoft.Common" %>
<%@ Register Src="/usercontrol/UserOrder/OrderCustomer.ascx" TagName="OrderCustomer"
    TagPrefix="uc1" %>
<asp:content id="AddOrderByRoute" runat="server" contentplaceholderid="ContentPlaceHolder1">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td align="left">
                                    ...
                                    <asp:Literal runat="server" id="lbd_price"></asp:Literal>
...
<script type="text/javascript">
    var AddOrderByRoute = {
        ...
        UpdatePrice: function(ref) {
            var obj = $("#" + AddOrderByRoute._getData(ref).ContainerID);
        ...
        SubmitForm: function(ref) {
            ...
            if ($.trim(obj.find("[id$=<%=txtTravel.ClientID %>]").val()) == "") {
                alert("请选择组团社!");
                return;
            }
            obj.find("#btnSave").html("提交中..");
            obj.find("#btnSave").unbind();
            $.newAjax({
                type: "POST",
                url: '/Order/RouteAgency/AddOrderByRoute.aspx?tourID=<%=Request.QueryString["tourID"] %>&dotype=save&v=' + Math.random(),
                data: obj.find("#btnSave").closest("form").serialize(),
                cache: false,
    ...
    function backCallFun(data,key) {
        var obj = $("#" + key);
        obj.find("#<%=hideTravelID.ClientID %>").val(data.comID);
        obj.find("#<%=txtTravel.ClientID %>").val(data.comName);
        obj.find("#<%=txtFzr.ClientID %>").val(data.conName);
        obj.find("#<%=txtFzrTel.ClientID %>").val(data.conTel);
    }
    $(function() {
        var obj = $("#<%=tblID %>");
        obj.find("#btnSave").click(function() {
            AddOrderByRoute.SubmitForm('<%=tblID %>');
        })
        obj.find("#<%=txtAdultCount.ClientID %>,#<%=txtChildCount.ClientID %>,#<%=txtOtherCount.ClientID %>,#<%=txtAddPrice.ClientID %>").blur(function() {

            AddOrderByRoute.UpdatePrice('<%=tblID %>');
        })

        obj.find("#btnSelectTravel").click(function() {
        Boxy.iframeDialog({ title: "选择单位", iframeUrl: "/RouteAgency/QueryTour.aspx?type=new&backCallFun=backCallFun&key=<%=tblID %>", height: 400, width: 760, draggable: false });
            return false;
        })
