...
namespace Adpost.Common.ExporPage
{
    ...
    public class ExporPageInfoSelect : c1
    {
        ...
        private string RegisterJs()
        {
            ...
            if (IsInitJs && HrefType == HrefTypeEnum.UrlHref && (PageStyleType == PageStyleTypeEnum.Select||PageStyleType == PageStyleTypeEnum.ClassicStyle))  //要初始化js，翻页通过urlhref跳转,并且是select样式的分页
            {   //URL方式跳转
                ...
                if (PageStyleType == PageStyleTypeEnum.ClassicStyle)
                {
                    strJs.AppendFormat("window.location.href='{0}_' + obj.value;", url.Replace("_#PageIndex#",""));
                }
                else
                {
                ...
                //初始化默认的事件
                AttributesEventAdd("onchange", "ExporPageInfoSelect_Change(this)", PageStyleType == PageStyleTypeEnum.Select ? 0 : 6);
            }
        ...
        protected override void Render(HtmlTextWriter output)
        {
            ...
            else if (PageStyleType == PageStyleTypeEnum.MQSeachFriendStyle)
            {
                #region MQ查找好友分页样式 周文超 2010-08-17

                tmpReturnValue.Append("<div class='diggPage'>");
                //构造页数前面的统计
                tmpReturnValue.AppendFormat("<span>第{0}页/共{1}页，共{2}条记录，</span>", CurrencyPage, intPageCount, intRecordCount);
                if (intPageCount == 1)
                {
                    tmpReturnValue.Append("<span class='pagetypenolink'> 首页 </span>"); //首页
                    tmpReturnValue.Append("<span class='pagetypenolink'> 上一页 </span>"); //上一页
                    tmpReturnValue.Append("<span class='pagetypenolink'> 下一页 </span>"); //下一页
                    tmpReturnValue.Append("<span class='pagetypenolink'> 末页 </span>"); //末页
                }
                else if (intPageCount > 1)
                {
                    if (CurrencyPage == 1)
                    ...
                    else if (CurrencyPage > 1)
                    {
                        if (HrefType == HrefTypeEnum.UrlHref)
                        {
                            tmpReturnValue.AppendFormat("<a href='{0}Page={1}'> 首页 </a>", PageLinkURL, 1); //首页
                            tmpReturnValue.AppendFormat("<a href='{0}Page={1}'> 上一页 </a>", PageLinkURL, CurrencyPage - 1); //上一页
                        }
                        else if (HrefType == HrefTypeEnum.JsHref)
                        {
                            //首页
                            tmpReturnValue.AppendFormat("<span gotoPage='{1}' {0}><a href='javascript:void(0);'> 首页 </a></span>", strJs, 1);
                            //上一页
                            tmpReturnValue.AppendFormat("<span gotoPage='{1}' {0}><a href='javascript:void(0);'> 上一页 </a></span>", strJs, CurrencyPage - 1);
                        }
                    }
                    if (CurrencyPage == intPageCount)
                    {
                        tmpReturnValue.Append("<span class='pagetypenolink'> 下一页 </span>"); //下一页
                        tmpReturnValue.Append("<span class='pagetypenolink'> 末页 </span>"); //末页
                    }
                    else if (CurrencyPage < intPageCount)
                    {
                        if (HrefType == HrefTypeEnum.UrlHref)
                        {
                            tmpReturnValue.AppendFormat("<a href='{0}Page={1}'> 下一页 </a>", PageLinkURL, CurrencyPage + 1); //下一页
                            tmpReturnValue.AppendFormat("<a href='{0}Page={1}'> 末页 </a>", PageLinkURL, intPageCount); //末页
                        }
                        else if (HrefType == HrefTypeEnum.JsHref)
                        {
                            //下一页
                            tmpReturnValue.AppendFormat("<span gotoPage='{1}' {0}><a href='javascript:void(0);'> 下一页 </a></span>", strJs, CurrencyPage + 1);
                            //末页
                            tmpReturnValue.AppendFormat("<span gotoPage='{1}' {0}><a href='javascript:void(0);'> 末页 </a></span>", strJs, intPageCount);
                        }
                    }
                }
                tmpReturnValue.Append(GetInput(intPageCount));

                #endregion
            }
            //add by xuqh 2011-4-11
            else if (PageStyleType == PageStyleTypeEnum.ClassicStyle)
            {
                tmpReturnValue.Append("<div class='pageNav'> ");
                ...
                {
                    //首页 上一页
                    tmpReturnValue.Append("<span href='#' class='disabled'>首页</span>");
                    tmpReturnValue.Append("<span class='na' class='disabled'>上一页</span>");

                    //中间数字
                    tmpReturnValue.Append(GetNumLink(intPageCount, strJs.ToString(),0));
                    ...
                    if (intPageCount > 1)
                    {
                        if (HrefType == HrefTypeEnum.UrlHref)
                        {
                            tmpReturnValue.AppendFormat("<a class='f12' href='{0}'> 下一页 </a>", BuildUrlStr(PageLinkURL, "2")); //下一页
                            tmpReturnValue.AppendFormat("<a class='disabled class='f12' href='{0}'>尾页</a> ", BuildUrlStr(PageLinkURL, intPageCount.ToString()));
                        }
                        else if (HrefType == HrefTypeEnum.JsHref)
                            tmpReturnValue.AppendFormat("<span gotoPage='2' {0}><a class='f12' href='javascript:void(0);'> 下一页 </a></span>", strJs); //下一页
                    }
                    //页数等于1
                    else if (intPageCount == 1)
                    {
                        tmpReturnValue.Append("<span class='f12' class='disabled' href='#'>下一页</span> "); //下一页
                        tmpReturnValue.Append("<span class='disabled class='f12' href='#'>尾页</span> ");
                    }
                }
                //当前页为最后1页
                else if (CurrencyPage == intPageCount)
                {
                    if (HrefType == HrefTypeEnum.UrlHref)
                    {
                        tmpReturnValue.AppendFormat("<a href='{0}' class='na' >首页</a>", BuildUrlStr(PageLinkURL, "1"));
                        tmpReturnValue.AppendFormat("<a class='na' href='{0}'> 上一页 </a>", BuildUrlStr(PageLinkURL, Convert.ToString(CurrencyPage - 1))); //上一页
                    }
                    else if (HrefType == HrefTypeEnum.JsHref)
                        tmpReturnValue.AppendFormat("<span gotoPage='{0}' {1}><a class='na' href='javascript:void(0);'> 上一页 </a></span>", CurrencyPage - 1, strJs);

                    //中间数字                  
                    tmpReturnValue.Append(GetNumLink(intPageCount, strJs.ToString(),0));

                    //下一页 尾页
                    tmpReturnValue.Append("<span class='f12' class='disabled'> 下一页 </span>");
                    tmpReturnValue.Append("<span class='disabled class='f12' href='#'>尾页</span> ");
                }
                //当为中间页的时候
                else
                {
                    ...
                    if (HrefType == HrefTypeEnum.UrlHref)
                    {
                        tmpReturnValue.AppendFormat("<a href='{0}' class='disabled'>首页</a>", BuildUrlStr(PageLinkURL, "1"));
                        tmpReturnValue.AppendFormat("<a class='na' href='{0}'> 上一页 </a>", BuildUrlStr(PageLinkURL, Convert.ToString(CurrencyPage - 1)));
                    }
                    else if (HrefType == HrefTypeEnum.JsHref)
                        tmpReturnValue.AppendFormat("<span gotoPage='{0}' {1}><a class='na' href='javascript:void(0);'> 上一页 </a></span>", CurrencyPage - 1, strJs);
                    ...
                    if (HrefType == HrefTypeEnum.UrlHref)
                    {
                        tmpReturnValue.AppendFormat("<a class='f12' href='{0}'> 下一页 </a>", BuildUrlStr(PageLinkURL, Convert.ToString(CurrencyPage + 1)));
                        tmpReturnValue.AppendFormat("<a class='disabled class='f12' href='{0}'>尾页</a> ", BuildUrlStr(PageLinkURL, intPageCount.ToString()));
                    }
                    else if (HrefType == HrefTypeEnum.JsHref)
                        tmpReturnValue.AppendFormat("<span gotoPage='{0}' {1}><a class='f12' href='javascript:void(0);'> 下一页 </a></span>", CurrencyPage + 1, strJs);
                }
                #endregion

                #region 构造下拉页数
                tmpReturnValue.Append("<span class='clear1'>转到</span>");
                tmpReturnValue.Append("<select");
            ...
            //add by xuqh 2011-4-11
            else if (PageStyleType == PageStyleTypeEnum.ContentStyle)
            {
                tmpReturnValue.Append("<div class='text-align:center; padding-top:20px; padding-bottom:20px; font-weight:700; font-size:14px;'>");
                ...
                if (CurrencyPage == 1)  //当前页为第1页
                {
                    ...
                    //中间页数的数字---------begin------                    
                    tmpReturnValue.Append(GetNumLink(intPageCount, strJs.ToString(),1));
                    //中间页数的数字----------end-----
            ...
            output.Write(strJsOut + strCss + tmpReturnValue.ToString());
        }
        ...
        /// <param name="strJs">通过js分页的时候的js事件</param>
        /// <param name="pageLinkURL">网页url链接</param>
        /// <param name="type">0为经典样式 1为内容分页</param>
        /// <returns></returns>
        private string GetPageNumLink(int pageIndex, int currentPage, HrefTypeEnum hrefType, string strJs, string pageLinkURL,int type)
        {
            StringBuilder tmpReturnValue = new StringBuilder();
            if (pageIndex == currentPage)
            {
                if (type == 0)
                {
                    tmpReturnValue.AppendFormat("<span style='color:#666'>{0}</span>", pageIndex);
                }
                else
                {
                    tmpReturnValue.AppendFormat("<span style='color:#666'>[{0}]</span>", pageIndex);   //当前页
                }
            }
            else
            {
        ...
        /// <param name="type">0为经典样式 1为内容分页</param>
        /// <returns></returns>
        private string GetNumLink(int intPageCount, string strJs,int type)
        {
            StringBuilder tmpReturnValue = new StringBuilder();
            int pageIndex = 1;   //页索引
            ...
            if (intPageCount <= PageLinkCount)   //直接输出每页
                for (pageIndex = 1; pageIndex <= intPageCount; pageIndex++)
                    tmpReturnValue.AppendFormat(GetPageNumLink(pageIndex, CurrencyPage, HrefType, strJs, PageLinkURL,type));
            else if (intPageCount > PageLinkCount)
            {
                ...
                if (isHasStartPoint)   //输出开始的...
                {
                    if (type == 0)
                    {
                        tmpReturnValue.AppendFormat("<span class='mor'>...</span>");
                    }
                    else
                    {
    ...
    public enum PageStyleTypeEnum
    {
        ...
        /// <summary>
        /// 经典样式-首页 上一页 1 2 3 ... 18 下一页 尾页 转到 8 
        /// </summary>
        ClassicStyle,

        /// <summary>
        /// 内容分页 上一页 1 2 3 ... 16 17 18 下一页
        /// </summary>
        ContentStyle
    }
